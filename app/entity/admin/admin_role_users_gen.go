...
func (orm *OrmAdminRoleUsers) TableName() string {
	return "admin_role_users"
}

func NewOrmAdminRoleUsers(txs ...*gorm.DB) *OrmAdminRoleUsers {
	var tx *gorm.DB
	if len(txs) == 0 {
		tx = providers.GetBean("mysql, admin").(*gorm.DB).Model(&AdminRoleUsers{})
	} else {
		tx = txs[0].Model(&AdminRoleUsers{})
	}
	return &OrmAdminRoleUsers{db: tx}
}
...
func (orm *OrmAdminRoleUsers) GetTableInfo() interface{} {
...
// Exists 检索对象是否存在
func (orm *OrmAdminRoleUsers) Exists() (bool, error) {
	dest := &struct {
		H int `json:"h"`
	}{}
...
func (orm *OrmAdminRoleUsers) Unscoped() *OrmAdminRoleUsers {
	orm.db.Unscoped()
	return orm
}
...
func (orm *OrmAdminRoleUsers) Group(name string) *OrmAdminRoleUsers {
	orm.db.Group(name)
	return orm
}
...
func (orm *OrmAdminRoleUsers) Select(query interface{}, args ...interface{}) *OrmAdminRoleUsers {
	orm.db.Select(query, args...)
	return orm
}

func (orm *OrmAdminRoleUsers) Sum(field string) int64 {
	type result struct {
		S int64 `json:"s"`
	}
	ret := result{}
...
func (orm *OrmAdminRoleUsers) Preload(query string, args ...interface{}) *OrmAdminRoleUsers {
...
func (orm *OrmAdminRoleUsers) Joins(query string, args ...interface{}) *OrmAdminRoleUsers {
