...
func (orm *OrmAdminRoles) TableName() string {
	return "admin_roles"
}

func NewOrmAdminRoles(txs ...*gorm.DB) *OrmAdminRoles {
	var tx *gorm.DB
	if len(txs) == 0 {
		tx = providers.GetBean("mysql, admin").(*gorm.DB).Model(&AdminRoles{})
	} else {
		tx = txs[0].Model(&AdminRoles{})
	}
	return &OrmAdminRoles{db: tx}
}
...
// Exists 检索对象是否存在
func (orm *OrmAdminRoles) Exists() (bool, error) {
	dest := &struct {
		H int `json:"h"`
	}{}
...
func (orm *OrmAdminRoles) Group(name string) *OrmAdminRoles {
	orm.db.Group(name)
	return orm
}
...
func (orm *OrmAdminRoles) Select(query interface{}, args ...interface{}) *OrmAdminRoles {
	orm.db.Select(query, args...)
	return orm
}

func (orm *OrmAdminRoles) Sum(field string) int64 {
	type result struct {
		S int64 `json:"s"`
	}
	ret := result{}
...
func (orm *OrmAdminRoles) Preload(query string, args ...interface{}) *OrmAdminRoles {
...
func (orm *OrmAdminRoles) Joins(query string, args ...interface{}) *OrmAdminRoles {
